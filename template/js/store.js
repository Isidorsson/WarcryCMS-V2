...
(function($)
{
	var methods =
	{
		...
		init : function(options)
		{
			...
			// Add-to-cart is handled directly from the store item markup.
			// This prevents Wowhead tooltip links from stealing the click.
			$this.off('click.storeAddDirect').on('click.storeAddDirect', '.store-add-to-cart', function(ev)
			{
				ev.preventDefault();
				ev.stopPropagation();
				var item = $(this).closest('li.store-clickable-item');
				if (item.length < 1) { return false; }
				$this.WarcryStore('shoppingCart').add({
					itemId: item.data('item-id'),
					entry: item.data('entry'),
					name: item.data('name'),
					storeItem: item.data('order'),
					icon: item.data('icon'),
			...
			$this.off('click.storeAddBox').on('click.storeAddBox', 'li.store-clickable-item div#item-cont', function(ev)
			{
				if ($(ev.target).closest('.item-price-coins').length) { return; }
				$(this).closest('li.store-clickable-item').find('.store-add-to-cart:first').trigger('click');
				return false;
			});
		...
		loadItem : function(options)
		{
			...
			$.get(
				...
				function(data)
				{
					...
					//set the icon
					$('.item-ico a#icon', element).css('background', 'url(https://wow.zamimg.com/images/wow/icons/large/' + icon.toLowerCase() + '.jpg)');
					//set the name
					$('.item-info p', element).html(name);
					//set the item info without overlaying the item name
					$('.item-info span#info', element).html(itemInfo);
					
					var $name = name;
					var $icon = icon.toLowerCase();
					var $quality = quality.toLowerCase();

					//Keep data attributes fresh for the direct add-to-cart handler bound in init.
					element.attr('data-name', $name);
					element.attr('data-icon', $icon);
					element.attr('data-quality', $quality);
					element.attr('data-item-id', $itemId);
					element.attr('data-entry', $entry);
					element.attr('data-order', $storeItem);
					element.find('#item-cont').css('cursor', 'pointer');
			...
			var add = function(data)
			{
				...
				cartElement.append(
					'<ul class="selected-item-row" id="cart-item-'+ $cartItemId +'" style="display: none;">' +
                    	'<li class="sitem-icon" id="'+ $quality +'"><a href="#" data-oldhref="http://old.wowhead.com/item='+ $entry +'" rel="item='+ $entry +'"><span style="background-image:url(\'https://wow.zamimg.com/images/wow/icons/large/'+ $icon +'.jpg\')"></span></a></li>' +
