...
<meta charset="utf-8"/>
<?php
// Force all relative template/resources links to resolve from the CMS root.
// This fixes broken forum images/icons when the URL is /forums/search.php,
// /forums/viewforum.php, or any other virtual forum path.
$__warcryBaseHref = rtrim($config['BaseURL'], '/') . '/';
echo '<base href="' . htmlspecialchars($__warcryBaseHref, ENT_QUOTES, 'UTF-8') . '">';
?>
...
<meta name="copyright" content="Copyright Warcry CMS 2026">
...
<style type="text/css">
	...
	#warcry-slider .warcry-bg-slide.active { opacity: 1; }
	#IndexTextFader {
		position: absolute;
		left: 0;
		top: 165px;
	...
	#IndexTextFader img.active { opacity: 1; }
</style>
</head>
<body>
 <center>
 ...
 <!--HEADER.End-->
	...
	if (defined('is_forums'))
	{
		...
		<div class="sub_head_image" align="center">
			...
			<div class="ForumsTextFader" id="IndexTextFader">
				<img src="\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/IndexSlider/boosts.png" style="opacity:0;"/>
				<img src="\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/IndexSlider/icc.png" style="opacity:0;"/>
				<img src="\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/IndexSlider/teleporter.png" style="opacity:0;"/>
				<img src="\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/IndexSlider/voters.png" style="opacity:0;"/>
				<img src="\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/IndexSlider/xprate.png" style="opacity:0;"/>
			</div>
			<!-- TextSlider.End -->';
	...
	else
	{
		...
		{
			echo '
			<!-- Index Text Fader + Image Slider -->
			<div class="slider" id="warcry-slider" align="center">
				<div class="warcry-bg-slide active" style="background-image:url(' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/test.png);"></div>
				<div class="warcry-bg-slide" style="background-image:url(' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/test2.png);"></div>

				<div id="IndexTextFader">
					<img class="active" src="' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/_BACKUP/boosts.png" alt="Character Boosts"/>
					<img src="' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/_BACKUP/icc.png" alt="Icecrown Citadel"/>
					<img src="' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/_BACKUP/teleporter.png" alt="Teleporter"/>
					<img src="' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/_BACKUP/voters.png" alt="Top Voters"/>
					<img src="' . rtrim($config['BaseURL'], '/') . '/template/style/images/IndexSlider/_BACKUP/xprate.png" alt="XP Rate Changer"/>
				</div>
			</div>

			<script type="text/javascript">
				$(function() {
					function rotateWarcryItems(selector, interval) {
						var $items = $(selector);
					...
					// Main background slider: 2 images, every 10 seconds
					rotateWarcryItems("#warcry-slider .warcry-bg-slide", 10000);

					// Text fader: 5 text images, every 5 seconds
					rotateWarcryItems("#IndexTextFader img", 5000);
				});
			</script>
			<!-- Index Slider.End -->';
		}
		else
		{
			echo '
			<div class="sub_head_image" align="center">
				<div style="background-image:url(\' . rtrim($config[\'BaseURL\'], \'/\') . \'/template/style/images/header-images/sub_image_1.jpg);"></div><!-- Random image -->
			</div>';
		}
	}
	?>
