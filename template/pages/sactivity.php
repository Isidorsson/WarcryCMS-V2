...
$res->bindValue(':acc', (int)$CURUSER->get('id'), PDO::PARAM_INT);
$res->execute();
				...
				<?php
					...
					if ($count > 0)
					{
						...
						while ($arr = $res->fetch())
						{
							...
							$res2->bindValue(':id', (int)$arr['itemId'], PDO::PARAM_INT);
							$res2->execute();
							...
							$itemQuality = strtolower($CORE->getItemQualityString($item['Quality']));
							$itemName = htmlspecialchars($item['name'], ENT_QUOTES, 'UTF-8');
							
							echo '
			                <li>
				                <p id="r-item"><span class="', $itemQuality, '" title="Store activity item">[', $itemName, ']</span></p>
