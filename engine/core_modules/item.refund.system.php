...
class ItemRefundSystem
{
	static public function AddRefundable($entry, $price, $currency, $character, $account = false)
	{
		...
		$insert = $DB->prepare("INSERT INTO `refundable_items` (`entry`, `price`, `currency`, `character`, `account`, `time`, `status`, `error`) VALUES (:entry, :price, :currency, :character, :account, :time, :status, :error);");
		$error = '';
		...
		$insert->bindParam(':error', $error, PDO::PARAM_STR);
		$insert->execute();
