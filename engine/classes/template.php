...
class Template
{
	...
	*/
	...
	private function AssetUrl($file)
	{
		if (preg_match('#^https?://#i', $file) || strpos($file, '//') === 0)
			return $file;

		return rtrim($this->config['BaseURL'], '/') . '/' . ltrim($file, '/');
	}
	...
	public function PrintCSS()
	{
		...
		{
			if (!empty($this->config['DisableMinify']))
			{
				foreach (array(RESOURCE_LOAD_PRIO_HIGH, RESOURCE_LOAD_PRIO_LOW) as $priority)
				{
					foreach ($this->CssFiles[$priority] as $css)
					{
						echo '<link rel="stylesheet" href="', ($css['remote'] ? $css['file'] : $this->AssetUrl($css['file'])), '" />';
					}
				}
				return;
			}
			...
			{
				if ($css['remote']) echo '<link rel="stylesheet" href="', $css['file'], '" />';
				else $string .= $css['file'].',';
			}
	...
	public function PrintJavascripts($array)
	{
		if (is_array($array) && (!empty($array[RESOURCE_LOAD_PRIO_HIGH]) || !empty($array[RESOURCE_LOAD_PRIO_LOW])))
		{
			if (!empty($this->config['DisableMinify']))
			{
				foreach (array(RESOURCE_LOAD_PRIO_HIGH, RESOURCE_LOAD_PRIO_LOW) as $priority)
				{
					foreach ($array[$priority] as $js)
					{
						echo '<script type="text/javascript" src="', ($js['remote'] ? $js['file'] : $this->AssetUrl($js['file'])), '"></script>';
					}
				}
				return;
			}
			...
			{
				if ($js['remote']) echo '<script type="text/javascript" src="', $js['file'], '"></script>';
				else $string .= $js['file'].',';
			}
