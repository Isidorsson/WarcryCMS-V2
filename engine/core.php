...
//constants
// Use absolute paths based on this file location so forum sub-pages like /forums/search.php
// do not break includes because of the current working directory.
$__warcryRootPath = dirname(__DIR__);
include_once $__warcryRootPath . '/engine/constants.php';

//General Classes
require_once $__warcryRootPath . '/configuration/config.php';
include_once $config['RootPath'] . '/engine/db_error_page.php';
...
class CORE
{
	...
	public function DatabaseConnection()
	{
		...
		if (!$this->db)
		{
			...
			{
				warcry_render_database_error('Database connection failed', 'Unable to connect to the website database.', 'configuration/database.php', !empty($this->config['DEBUG']), $e->getMessage());
			}
	...
	public function AuthDatabaseConnection()
	{
		...
		if (!$this->auth_db)
		{
			...
			{
				warcry_render_database_error('Authentication failed. Connection failed.', 'Unable to connect to the Auth database.', 'configuration/authentication.php', !empty($this->config['DEBUG']), $e->getMessage());
			}
	...
	public function convertCooldown($timestamp)
	{
	  global $CORE;
		...
		//get the seconds, minutes, hours and days. Use integer math for PHP 8 compatibility.
		$difference = (int)$difference;
		$seconds = $difference % 60;
		$minutes = intdiv($difference, 60) % 60;
		$hours = intdiv($difference, 60 * 60) % 24;
		$days = intdiv($difference, 24 * 60 * 60) % 30;
		
		return array('seconds' => $seconds, 'minutes' => $minutes, 'hours' => $hours, 'days' => $days, 'int' => $difference, 'timestamp' => (int)$timestamp);
	}

	public function percent($num_amount, $num_total)
	{
		$num_amount = (float)$num_amount;
		$num_total = (float)$num_total;
		if ($num_total <= 0)
		{
			return 0;
		}
		$count = round(($num_amount / $num_total) * 100);
		if ($count < 0) $count = 0;
	...
	public function ExecuteSoapCommand($command, $realmid)
	{
	    ...
	    {
	        if (isset($this->config['RootPath'])) {
	            @file_put_contents($this->config['RootPath'] . '/cache/purchase_delivery_error.log', '[' . date('Y-m-d H:i:s') . '] SOAP exception | realm=' . $realmid . ' | command=' . $command . ' | ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
	        }
