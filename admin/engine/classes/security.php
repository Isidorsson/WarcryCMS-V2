...
//Dont erase those... :]
// PHP 8+ compatibility: get_magic_quotes_gpc() and each() were removed.
// Keep the old legacy variables available without calling removed functions.
...
if (!function_exists('warcry_legacy_addslashes')) {
    function warcry_legacy_addslashes($value) {
        if (is_array($value)) {
            foreach ($value as $key => $item) {
                $value[$key] = warcry_legacy_addslashes($item);
            }
            return $value;
        }
...
$warcry_magic_quotes_enabled = function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc();
if (!$warcry_magic_quotes_enabled) {
    if (isset($HTTP_GET_VARS) && is_array($HTTP_GET_VARS)) {
        $HTTP_GET_VARS = warcry_legacy_addslashes($HTTP_GET_VARS);
    }
    if (isset($HTTP_POST_VARS) && is_array($HTTP_POST_VARS)) {
        $HTTP_POST_VARS = warcry_legacy_addslashes($HTTP_POST_VARS);
    }
    if (isset($HTTP_COOKIE_VARS) && is_array($HTTP_COOKIE_VARS)) {
        $HTTP_COOKIE_VARS = warcry_legacy_addslashes($HTTP_COOKIE_VARS);
    }
}
