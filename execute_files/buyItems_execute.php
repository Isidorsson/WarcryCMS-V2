...
// Do not block purchases only because SOAP is offline. sendItems() has a direct DB mail fallback.
if (!$charName)
{
	...
	if ($chars->setRealm($RealmId))
	{
		...
		if ($chars->isMyCharacter(false, $charName))
		{
			...
			foreach ($items as $index => $data)
			{
				...
				$res = $DB->prepare("SELECT id, entry, silver, gold FROM `store_items` WHERE `id` = :item AND " . warcry_realm_match_sql('`realm`') . " LIMIT 1;");
				...
				if ($res->rowCount() > 0)
				{
					...
					if ($currency == 'gold' or $currency == 'silver')
					{
						...
						if ($hasMoney)
						{
							...
							$insert->bindValue(':acc', $accountId, PDO::PARAM_INT);
			...
			if ($itemsString)
			{
				...
				if ($sentMail === true)
				{
					...
					$update->bindValue(':account', (int)$CURUSER->get('id'), PDO::PARAM_INT);	
					$update->execute();
